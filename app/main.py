...
@app.get("/debug/recon")
async def debug_recon(action: str = "start",
                      url: str = "https://www.chat-avenue.com/adultchat.html", key: str = ""):
    """Temporary recon: spin up a BU Cloud browser (clears Cloudflare), hand back a live_url
    for a human to drive, and dump the DOM of every page in the context (popups included) so we
    can map a new platform. Key-gated. action = start | inspect | stop."""
    global _recon_worker
    ...
    if action == "start":
        w = BotWorker({"username": "ReconBot", "id": "recon"})
        w.agent_id = "ReconBot"
        if not await browser_manager._provision_and_connect(w):
            return {"error": "provision failed"}
        try:
            await w._page.goto(url, wait_until="domcontentloaded", timeout=45000)
        except Exception as e:
            logger.warning(f"[recon] goto error: {e}")
        _recon_worker = w
        return {"live_url": w.live_url, "browser_id": w.browser_id, "url": w._page.url,
                "hint": "open live_url, do the guest login, then call action=inspect"}

    if action == "inspect":
        w = _recon_worker
        if not w or not w._page:
            return {"error": "no recon session — call action=start first"}
        try:
            pages = list(w._page.context.pages)
        except Exception:
            pages = [w._page]
        probe = """() => {
            const q = (sel) => Array.from(document.querySelectorAll(sel)).slice(0,12).map(e => ({
                tag:e.tagName, id:e.id||'', cls:(e.className||'').toString().slice(0,70),
                ph:e.placeholder||'', txt:(e.textContent||'').trim().slice(0,40)}));
            return {
                title: document.title,
                iframes: Array.from(document.querySelectorAll('iframe')).map(f=>f.src).slice(0,6),
                inputs: q('input, textarea, [contenteditable=\\'true\\']'),
                pmHints: q('[class*=private i],[class*=whisper i],[class*=pm i],[class*=dm i],[id*=private i],[title*=private i],[onclick*=private i]'),
                roomHints: q('[class*=room i],[class*=channel i],[id*=room i]'),
                userHints: q('[class*=userlist i],[class*=member i],[class*=nick i]'),
        ...
        for pg in pages:
            try:
                ...
                out.append({"page_url": pg.url, **info})
            except Exception as e:
                out.append({"page_url": getattr(pg, "url", "?"), "error": str(e)[:200]})
        return {"page_count": len(pages), "pages": out}

    if action == "stop":
        w = _recon_worker
        if w:
            try:
                await browser_manager._teardown_browser(w)
            except Exception:
                pass
        _recon_worker = None
        return {"stopped": True}
    return {"error": "unknown action (start|inspect|stop)"}
